	 ...
	 */
	 EntityRelation: function (state, source, target, points, result)
	 {
		const view = state.view;
	 	...
	 	const segment = mxUtils.getValue(state.style,
	...
	Loop: function (state, source, target, points, result)
	{
		...
		if (source != null)
		{
			...
			let dy = 0;
		 	...
		 	const seg = mxUtils.getValue(state.style, mxConstants.STYLE_SEGMENT,
	...
	OrthConnector: function(state, source, target, points, result)
	{
		...
		for (var i = 0; i <= currentIndex; i++)
		{
			if (i == currentIndex)
			{
				...
				// odd.
				const targetOrientation = (dir[1] & (mxConstants.DIRECTION_MASK_EAST | mxConstants.DIRECTION_MASK_WEST)) > 0 ? 0
						: 1;
