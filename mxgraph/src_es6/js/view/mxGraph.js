...
class mxGraph extends mxEventSource {
                    ...
                    const disconnectTerminal = mxUtils.bind(this, function(edge, source)
                    {
                        ...
                        if (geo != null)
                        {
                            ...
                            if (state != null)
                            {
                                ...
                                let tmp = state.getVisibleTerminal(source);
                                let connected = false;
                                ...
                                if (connected)
                                {
                                    let dx = tr.x;
                                    let dy = tr.y;
                                    const parentState = this.view.getState(this.model.getParent(edge));
    ...
    getOutlineConstraint(point, terminalState, me) {
        if (terminalState.shape != null)
        {
            ...
            const x = Math.round((point.x - bounds.x) * 1000 / bounds.width) / 1000;
            const y = Math.round((point.y - bounds.y) * 1000 / bounds.height) / 1000;
