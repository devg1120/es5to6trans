...
const mxUtils =
{
	...
	{
		const value = mxUtils.getValue(terminal.style, mxConstants.STYLE_PORT_CONSTRAINT,
	...
	findNearestSegment: function(state, x, y)
	{
		...
		if (state.absolutePoints.length > 0)
		{
			...
			for (let i = 1; i < state.absolutePoints.length; i++)
			{
				...
				const dist = mxUtils.ptSegDistSq(last.x, last.y,
	...
	getPerimeterPoint: function (pts, center, point)
	{
		...
		{
			const pt = mxUtils.intersection(pts[i].x, pts[i].y, pts[i + 1].x, pts[i + 1].y,
	...
	error: function(message, width, close, icon)
	{
		...
		const warn = new mxWindow(mxResources.get(mxUtils.errorResource) ||
